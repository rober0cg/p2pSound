...
public class P2PSoundCLI {
    ...
    private static final String help = 
        ...
        "\n\t -Q playerQueueSize, para especificar primer envío distinto de " +AppParams.packetFactor1+
        "\n\t -h, esta ayuda"+
        ""
    ;
...
// si no se especifican, los "por defecto"
        ...
        if ( playerQueueSize==0 ) playerQueueSize=AppParams.packetFactor1;
    ...
    public static void main(String[] args) throws Exception {
        ...
        if ( playerQueueSize != AppParams.packetFactor1 ) AppParams.setPlayerQueueSize ( playerQueueSize );
        AppParams.show();
        ...
        Thread tSendREC = new Thread(new Runnable() {
            public void run() {
                ...
                // Primer envío con buffer mayor
                {
                    ...
                    int s = sDAT.send( rt.getbPacketBuf(), rt.getnPacketLen() );
                    if ( s<=0 ) {
        ...
        Thread tPlayRCV = new Thread(new Runnable() {
            public void run() {
                ...
                // Primera recepción con buffer mayor
                {
                    ...
                    int r = sDAT.recv( rt.getbPacketBuf(), rt.getnPacketLen() );
                    if ( r<=0 ) {
